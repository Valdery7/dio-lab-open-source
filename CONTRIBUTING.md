...
```bash
git clone https://github.com/valdery7/dio-lab-open-source.git
```
...
> Exemplo: `git checkout -b feat/community/valdery7`
...
Utilize o comando `git add community/valdery7.md` para adicionar sua alteração (nesse caso o arquivo markdown criado)  à "staging area" no Git.

### 6) Crie um Commit
Crie um commit e adicione a mensagem indicando a adição do seu perfil:
```bash
git commit -m"feat: add valdery7 profile"
```
>[!IMPORTANT]
...
Envie as alterações realizadas no seu repositório local para a branch `feat/community/valdery7` no seu repositório remoto com o comando:
```bash
git push origin feat/community/valdery7
```
>[!WARNING]
...
- Seu PR deve modificar apenas o arquivo community/valdery7.md (dê uma olhadinha na aba "Files changed");
